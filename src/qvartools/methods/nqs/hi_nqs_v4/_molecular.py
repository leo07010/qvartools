...
try:
    from ._hamiltonian_base import Hamiltonian
except ImportError:
...
class MolecularHamiltonian(Hamiltonian):
    ...
    def enable_full_sd_enumeration(self):
        """Re-precompute excitation indices WITHOUT h2e/h1 nonzero filter.

        This causes get_connections_vectorized_batch to enumerate ALL S+D
        combinatorially, including cross-irrep (symmetry-forbidden) doubles
        that have h_matrix_element = 0. These dets can still appear in CI
        eigenvectors via indirect couplings (e.g., second-order through
        same-irrep intermediate dets).

        WARNING: connection output grows ~10x. Memory and wall time scale.
        Default v4 path uses filtered enumeration (faster, drops cross-irrep).
        """
        self.enumerate_zero_h = True
        # Rebuild h1_offdiag with NO threshold (just exclude diagonal)
        n_orb = self.n_orbitals
        device = self.device
        eye_mask = torch.eye(n_orb, device=device, dtype=torch.bool)
        not_diag = ~eye_mask
        self.h1_offdiag_indices = torch.nonzero(not_diag)
        self.h1_offdiag_values = self.h1e[not_diag]
        # Rebuild dependent precomputes
        self._precompute_single_excitation_data()
        self._precompute_excitation_indices()
        # Also rebuild sparse h2e dictionary
        self._precompute_sparse_h2e()
        print(f"[H] Enabled full S+D enumeration: "
              f"singles={len(self._single_p)}, same-spin D={len(self._double_same_p)}, "
              f"αβ D={len(self._double_ab_p)}", flush=True)
    ...
    def _precompute_excitation_indices(self):
        ...
        If self.enumerate_zero_h is True, skips the nonzero filters on h1/h2e
        and keeps ALL combinatorial S+D index pairs. This is necessary to
        enumerate cross-irrep (symmetry-forbidden) excitations that have h=0
        but still appear in the CI eigenvector via indirect couplings.
        """
        n_orb = self.n_orbitals
        device = self.device
        enumerate_full = bool(getattr(self, "enumerate_zero_h", False))

        # === SINGLE EXCITATIONS ===
        # All (p, q) pairs where p != q.
        # Default: filter to h1e[p,q] != 0 (via h1_offdiag_indices already filtered).
        # enumerate_zero_h: keep ALL off-diagonal pairs.
        if enumerate_full:
            eye = torch.eye(n_orb, device=device, dtype=torch.bool)
            not_diag_mask = ~eye
            full_indices = torch.nonzero(not_diag_mask)  # (n_orb*(n_orb-1), 2)
            single_p = full_indices[:, 0].to(device)
            single_q = full_indices[:, 1].to(device)
            single_h1e = self.h1e[single_p, single_q].to(device)
        else:
        ...
        if n_occ_pairs > 0 and n_virt_pairs > 0:
            ...
            # Filter by non-zero values (skip if enumerate_zero_h)
            if enumerate_full:
                self._double_same_q = double_q
                self._double_same_s = double_s
                self._double_same_p = double_p
                self._double_same_r = double_r
                self._double_same_h2e = double_h2e_same
            else:
        ...
        if n_orb > 0:
            ...
            # Filter by non-zero (skip if enumerate_zero_h to enumerate cross-irrep doubles)
            if enumerate_full:
                self._double_ab_q = ab_q
                self._double_ab_s = ab_s
                self._double_ab_p = ab_p
                self._double_ab_r = ab_r
                self._double_ab_h2e = ab_h2e
            else:
        ...
        try:
            ...
            try:
                try:
                    ...
                    log_allocation = lambda *a, **kw: None
            except ImportError:
                log_allocation = None
...
def compute_molecular_integrals(
    ...
    try:
        ...
        try:
            try:
                ...
                load_integrals, save_integrals = None, None
            _cache_mod = True
        except ImportError:
            load_integrals = None
            save_integrals = None
    ...
    if _cache_mod and use_cache and cas is None:
    ...
    # Save to cache
    if use_cache and save_integrals is not None:
        try:
