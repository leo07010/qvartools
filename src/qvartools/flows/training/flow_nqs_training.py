"""
Flow-NQS Co-Training for Normalizing Flows.
...
This is DIFFERENT from :mod:`physics_guided_training` which already exists.
This file contains :class:`FlowNQSConfig` (co-training config),
:class:`PhysicsGuidedFlowTrainer` (the main NF-NQS co-training loop),
and the :func:`create_physics_guided_trainer` factory.
...
try:
    ...
    ConnectionCache = None  # type: ignore[assignment,misc]

# Optional: compute_max_cache_size helper (may not be present yet)
try:
...
try:
    ...
    config_integer_hash = None  # type: ignore[assignment]
...
@dataclass
class FlowNQSConfig:
    """Configuration for flow-NQS co-training.

    This is distinct from
    :class:`~qvartools.flows.training.physics_guided_training.PhysicsGuidedConfig`
    which drives the simpler physics-guided trainer.  :class:`FlowNQSConfig`
    controls the full NF-NQS co-training loop with subspace diagonalisation,
    essential-configuration injection, and connection caching.

    Parameters
    ----------
    samples_per_batch : int, optional
        Number of flow samples per mini-batch (default ``2000``).
    num_batches : int, optional
        Number of mini-batches per epoch (default ``1``).
    nqs_chunk_size : int, optional
        Maximum batch size for NQS evaluation during local-energy
        computation (default ``16384``).
    flow_lr : float, optional
        Learning rate for the flow optimiser (default ``5e-4``).
    nqs_lr : float, optional
        Learning rate for the NQS optimiser (default ``1e-3``).
    num_epochs : int, optional
        Maximum number of training epochs (default ``500``).
    min_epochs : int, optional
        Minimum epochs before convergence / early-stopping checks
        activate (default ``150``).
    convergence_threshold : float, optional
        Training stops when the unique-configuration ratio drops below
        this value after ``min_epochs`` (default ``0.35``).
    teacher_weight : float, optional
        Weight of the teacher (KL) loss (default ``1.0``).
    physics_weight : float, optional
        Weight of the REINFORCE energy-gradient loss (default ``0.1``).
    entropy_weight : float, optional
        Weight of the entropy regularisation (default ``0.05``).
    use_energy_baseline : bool, optional
        Whether to subtract a running baseline from the energy for
        variance reduction (default ``True``).
    use_accumulated_energy : bool, optional
        Whether to compute energy from the accumulated basis via
        diagonalisation (default ``True``).
    max_accumulated_basis : int, optional
        Maximum number of configurations in the accumulated basis
        (default ``2048``).
    accumulated_energy_interval : int, optional
        Compute accumulated energy every *N* epochs (default ``50``).
    prune_basis_threshold : float, optional
        Probability threshold below which basis states may be pruned
        (default ``1e-6``).
    ema_decay : float, optional
        Exponential moving average decay for energy tracking
        (default ``0.95``).
    initial_temperature : float, optional
        Starting temperature for flow annealing (default ``1.0``).
    final_temperature : float, optional
        Final temperature after annealing (default ``0.3``).
    temperature_decay_epochs : int, optional
        Number of epochs over which to anneal temperature
        (default ``400``).
    use_connection_cache : bool, optional
        Whether to cache Hamiltonian connections (default ``True``).
    max_cache_size : int, optional
        Maximum entries in the connection cache (default ``100000``).
    cache_warmup : bool, optional
        Pre-populate cache with HF neighbourhood (default ``True``).
    cache_warmup_excitation_level : int, optional
        Excitation level for cache warmup: 1 = singles, 2 = singles +
        doubles (default ``2``).
    use_torch_compile : bool, optional
        Apply ``torch.compile`` for faster NQS evaluation
        (default ``True``).
    use_parallel_connections : bool, optional
        Compute Hamiltonian connections in parallel (default ``True``).
    parallel_workers : int, optional
        Number of parallel workers for connection computation
        (default ``8``).
    early_stopping_patience : int, optional
        Stop if energy does not improve for this many epochs
        (default ``100``).
    early_stopping_threshold : float, optional
        Minimum improvement in Hartree to reset patience counter
        (default ``0.001``).
    use_subspace_energy : bool, optional
        Use subspace diagonalisation for energy instead of local energy
        (default ``True``).
    max_subspace_diag_size : int, optional
        Maximum basis size for subspace diagonalisation
        (default ``2048``).
    subspace_energy_interval : int, optional
        Compute subspace energy every *N* epochs (default ``1``).
    inject_essential_configs : bool, optional
        Inject HF + singles + doubles into the basis (default ``True``).
    always_include_hf : bool, optional
        Always include the HF state in subspace energy computation
        (default ``True``).
    include_singles_in_basis : bool, optional
        Include single excitations in essential configs
        (default ``True``).
    include_doubles_in_basis : bool, optional
        Include double excitations in essential configs
        (default ``True``).

    Examples
    --------
    >>> cfg = FlowNQSConfig(num_epochs=100, physics_weight=0.2)
    >>> cfg.num_epochs
    100
    """
...
class PhysicsGuidedFlowTrainer:
    """Mixed-objective trainer for joint flow + NQS co-training.

    Combines three loss terms with configurable weights:

    * **Teacher loss**: ``KL(NQS || Flow)`` --- cross-entropy that trains
      the flow to match the NQS probability distribution.
    * **Physics loss**: REINFORCE energy-gradient signal that encourages
      sampling low-energy configurations.
    * **Entropy bonus**: ``-H[p_flow]`` (negative entropy) --- prevents
      mode collapse by encouraging distribution spread.

    The trainer also:

    * Accumulates unique configurations into a growing basis set.
    * Performs subspace diagonalisation for energy estimation
      (paper Sections 3.2--3.3).
    * Anneals the flow temperature via exponential decay.
    * Injects essential (HF + singles + doubles) configurations.
    * Caches Hamiltonian connections for efficiency.
    * Supports early stopping on energy plateau.

    Parameters
    ----------
    flow : nn.Module
        The normalizing flow sampler.  Must implement ``sample(batch_size)``
        and ``log_prob(configs)``.
    nqs : nn.Module
        The neural quantum state.  Must implement ``log_amplitude(x)``.
    hamiltonian : object
        The Hamiltonian operator with ``get_connections``,
        ``diagonal_elements_batch``, and ``matrix_elements`` methods.
    config : FlowNQSConfig
    ...
    flow_optimizer : torch.optim.AdamW
        Optimiser for flow parameters.
    nqs_optimizer : torch.optim.AdamW
        Optimiser for NQS parameters.
    energy_ema : float or None
        Exponential moving average of the energy.
    connection_cache : object or None
        Cache for Hamiltonian connections.
    history : dict
        Training history keyed by metric name.

    Examples
    --------
    >>> trainer = PhysicsGuidedFlowTrainer(flow, nqs, hamiltonian,
    ...     config=FlowNQSConfig(num_epochs=50), device="cpu")
    ...
    def _generate_essential_configs(self) -> torch.Tensor:
        ...
        The HF state has the largest coefficient (~0.9) in the ground
        state; singles capture orbital relaxation; doubles capture
        electron correlation (most important for chemical accuracy).

        Returns
        -------
        torch.Tensor
            Unique essential configurations, shape
            ``(n_essential, num_sites)``.
        """
        config = self.config
    ...
    def _warmup_cache_with_hf_neighborhood(self) -> None:
        """Pre-populate the connection cache with HF neighbourhood configs.

        Computing connections for the HF state and its single/double
        excitations up front dramatically improves cache hit rate because
        these are the most physically important configurations and the
        flow explores them heavily during early training.
        """
    ...
    def train(self) -> Dict[str, list]:
        ...
        based on convergence of the unique-configuration ratio and energy
        plateau detection.
        ...
        dict
            ...
            - ``"accumulated_energies"`` : list of float
            - ``"teacher_losses"`` : list of float
            - ``"physics_losses"`` : list of float
            - ``"entropy_values"`` : list of float
            - ``"unique_ratios"`` : list of float
            - ``"basis_sizes"`` : list of int
            - ``"cache_hit_rates"`` : list of float
        """
        config = self.config

        print("Starting physics-guided NF-NQS training")
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        loss (teacher + physics - entropy), updates both flow and NQS
        parameters, and accumulates unique configurations into the basis.
        ...
        -------
        dict
            Epoch metrics with keys ``"energy"``, ``"teacher_loss"``,
            ``"physics_loss"``, ``"entropy"``, ``"unique_ratio"``, and
            optionally ``"accumulated_energy"`` and ``"energy_ema"``.
        """
        config = self.config
        self.flow.train()
        self.nqs.train()
        ...
        for batch_idx in range(config.num_batches):
            ...
            if config.use_subspace_energy:
                compute_subspace_this_epoch = (
                    epoch % config.subspace_energy_interval == 0
                    or epoch == 0
                    or not hasattr(self, "_cached_subspace_energy")
                )
        ...
        if self.energy_ema is None:
            ...
            self.energy_ema = (
                config.ema_decay * self.energy_ema
        ...
        if (
            config.use_accumulated_energy
            and epoch % config.accumulated_energy_interval == 0
            and self.accumulated_basis is not None
        ):
    ...
    def _compute_subspace_energy(
        ...
        r"""Compute energy by diagonalising H in the sampled subspace.

        Implements the paper's method (Sections 3.2--3.3): the energy is
        the lowest eigenvalue of the Hamiltonian restricted to the
        sampled subspace *S*.  Essential configurations (HF + singles +
        doubles) are always included to anchor the subspace around the
        ground state.

        Parameters
        ----------
        configs : torch.Tensor
            Sampled configurations, shape ``(n_configs, num_sites)``.
        nqs_probs : torch.Tensor
            NQS probabilities for each configuration, shape
            ``(n_configs,)``.

        Returns
        -------
        torch.Tensor
            Ground-state energy in the subspace (scalar).

        Notes
        -----
        When the subspace exceeds ``max_subspace_diag_size``, the method
        retains all essential configurations and fills the remaining
        budget with the highest-probability sampled states.
        """
        config = self.config
            ...
            if self._essential_configs is not None:
                ...
                if n_essential >= config.max_subspace_diag_size:
                    ...
                    _, top_idx = torch.topk(
                        diag_energies[1:],
                        min(n_select, len(diag_energies) - 1),
                        largest=False,
                    )
                    ...
                    if config_integer_hash is not None:
                        ...
                        if non_ess_mask.any() and max_sampled > 0:
                            ...
                            configs = torch.cat(
                                [self._essential_configs, sampled_configs], dim=0
                            )
                            configs = torch.unique(configs, dim=0)
                        else:
                            configs = self._essential_configs
                    else:
                        # Fallback without config_integer_hash
                        configs = self._essential_configs
            else:
    ...
    def _compute_local_energies(
        ...
        r"""Compute local energies :math:`E_{\mathrm{loc}}(x)` for each configuration.

        .. math::

            E_{\mathrm{loc}}(x) = H_{x,x}
            + \sum_{x' \neq x} H_{x,x'}\,
              \frac{\psi(x')}{\psi(x)}
        ...
        nqs_chunk_size : int, optional
            Maximum batch size for NQS forward passes
            (default ``16384``).
        diagonal_only : bool, optional
            If ``True``, return only diagonal Hamiltonian elements for a
            fast warmup estimate (default ``False``).

        Returns
        -------
        torch.Tensor
            Local energies, shape ``(n_configs,)``.
        """
        n_configs = len(configs)
        ...
        with torch.no_grad():
            ...
            log_psi_connected = torch.empty(
                total_connections, device=self.device, dtype=torch.float64
            )
    ...
    def _get_connections_batch(
        ...
        cache, vectorised batch, parallel workers, or serial fallback.

        Parameters
        ----------
        configs : torch.Tensor
            Configurations, shape ``(n_configs, num_sites)``.

        Returns
        -------
        connected : torch.Tensor
            All connected configurations, shape ``(N_total, num_sites)``.
        elements : torch.Tensor
            Corresponding matrix elements, shape ``(N_total,)``.
        orig_indices : torch.Tensor
            Index into *configs* for each connected config, shape
            ``(N_total,)``.
        """
        config = self.config
        ...
        if self.connection_cache is not None:
            ...
            use_cache = (
                total_queries < 100
                or self.connection_cache.hit_rate > 0.1
                or len(self.connection_cache) < 1000
            )
        ...
        elif config.use_parallel_connections and hasattr(
    ...
    def _compute_flow_loss(
        ...
        ----------
        all_configs : torch.Tensor
            All sampled configurations, shape ``(batch, num_sites)``.
        unique_configs : torch.Tensor
            Unique configurations, shape ``(n_unique, num_sites)``.
        nqs_probs : torch.Tensor
            Normalised NQS probabilities, shape ``(n_unique,)``.
        local_energies : torch.Tensor
            Local energies for each unique config, shape ``(n_unique,)``.
        energy : torch.Tensor
            Scalar energy estimate (subspace or local-energy mean).
        ...
        components : dict
            Individual loss components with keys ``"teacher"``,
            ``"physics"``, ``"entropy"``.
        """
        config = self.config

        # Get flow log-probabilities directly
        ...
        scaled_loss = (
            config.teacher_weight * teacher_loss + config.physics_weight * physics_loss
        ) * energy_scale
    ...
    def _compute_nqs_loss(
        ...
        """Compute the NQS energy-minimisation loss.

        Uses a REINFORCE-style policy-gradient estimator with
        variance reduction via mean-energy centering.
        ...
        configs : torch.Tensor
            Configurations, shape ``(n_configs, num_sites)``.
        probs : torch.Tensor
            Detached NQS probabilities, shape ``(n_configs,)``.
        local_energies : torch.Tensor
            Local energies, shape ``(n_configs,)``.
    ...
    def _update_accumulated_basis(self, new_configs: torch.Tensor) -> None:
        """Add new configurations to the accumulated basis.

        Uses hash-based deduplication for O(n) complexity when
        ``config_integer_hash`` is available, falling back to
        ``torch.unique`` (O(n log n)) otherwise.  Essential
        configurations (HF + singles + doubles) are always retained.
        ...
        if config_integer_hash is None:
            # Fallback: use torch.unique
        ...
        if self.accumulated_basis is None:
            ...
            if new_unique_indices:
                self.accumulated_basis = torch.cat(
                    [self.accumulated_basis, new_configs[new_unique_indices]], dim=0
                )
    ...
    def _compute_accumulated_energy(self) -> float:
        """Compute energy in the accumulated basis via diagonalisation.

        For bases larger than 3000 configurations, uses direct sparse
        matrix construction to avoid O(n^2) dense allocation.  Falls
        back to dense ``numpy.linalg.eigh`` for small bases.

        Returns
        -------
        float
            Lowest eigenvalue of the Hamiltonian restricted to the
            accumulated basis, or ``inf`` if the basis is empty.
        """
        ...
        with torch.no_grad():
            # Direct sparse path for large bases
            SPARSE_ACCUM_THRESHOLD = 3000
            if n_basis > SPARSE_ACCUM_THRESHOLD and hasattr(
                self.hamiltonian, "get_sparse_matrix_elements"
            ):
                import gc
                ...
                try:
                    ...
                    H_coo = coo_matrix(
                        (vals_np, (rows_np, cols_np)), shape=(n_basis, n_basis)
                    )
                    del rows_np, cols_np, vals_np
                    diag_np = (
                        self.hamiltonian.diagonal_elements_batch(basis)
                        .cpu()
                        .numpy()
                        .astype(np.float64)
                    )
            ...
            if n_basis > 500:
                try:
                    ...
                    print(
                        f"WARNING: sparse eigsh failed in _compute_accumulated_energy "
                        f"({e}), falling back to dense"
                    )
...
def create_physics_guided_trainer(
    ...
    **kwargs: Any,
    ...
    """Create a :class:`PhysicsGuidedFlowTrainer` with sensible defaults.

    Convenience factory that constructs a :class:`FlowNQSConfig` from
    the supplied weights and keyword arguments, then instantiates the
    trainer.

    Parameters
    ----------
    flow : nn.Module
        Normalizing flow model (e.g.
        :class:`~qvartools.flows.networks.particle_conserving_flow.ParticleConservingFlowSampler`).
    nqs : nn.Module
        Neural quantum state model (e.g.
        :class:`~qvartools.nqs.architectures.dense.DenseNQS`).
    hamiltonian : object
        System Hamiltonian with ``get_connections``,
        ``diagonal_elements_batch``, and ``matrix_elements`` methods.
    device : str, optional
        Torch device string (default ``"cuda"``).
    teacher_weight : float, optional
        Weight for the teacher (KL) signal (default ``1.0``).
    physics_weight : float, optional
        Weight for the REINFORCE energy signal (default ``0.1``).
    entropy_weight : float, optional
        Weight for the entropy bonus (default ``0.05``).
    **kwargs
        Additional keyword arguments forwarded to :class:`FlowNQSConfig`.

    Returns
    -------
    PhysicsGuidedFlowTrainer
        Configured trainer instance ready for ``train()``.

    Examples
    --------
    >>> trainer = create_physics_guided_trainer(
    ...     flow, nqs, hamiltonian, device="cpu",
    ...     num_epochs=100, physics_weight=0.2,
    ... )
    >>> history = trainer.train()
    """
    config = FlowNQSConfig(
